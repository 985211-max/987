...
9. [如何让其他设备（手机/其他电脑）登录](#9-如何让其他设备手机其他电脑登录)
...
## 9. 如何让其他设备（手机/其他电脑）登录

> 默认情况下，系统只允许**运行代码的这台电脑**自己访问，这就是为什么同学/同事用自己的手机或电脑登录不了的原因。  
> 按照下面的步骤配置，让同一 Wi-Fi / 局域网内的所有设备都能访问。
...
2. 在命令行中输入：
   ...
   ipconfig
   ```
3. 找到"无线局域网适配器 Wi-Fi"或"以太网适配器"下面的 **IPv4 地址**，格式类似 `192.168.1.100`

**macOS：**
1. 打开"系统设置" → "网络" → 选择当前连接的网络
2. 或在终端输入：
   ```bash
   ipconfig getifaddr en0
   ```

**记下这个 IP 地址**，下面要用到（本教程以 `192.168.1.100` 为例，替换成你的实际 IP）。

---

### ⚙️ 第二步：修改后端配置文件 `.env`

打开 `backend/.env`，找到 `CLIENT_URL` 这一行，在原来 `localhost:3000` 的基础上，**追加你的 IP 地址**（用英文逗号分隔）：
...
> ⚠️ 将 `192.168.1.100` 替换成**第一步**中查到的你的实际 IP 地址。

保存文件后，**重启后端**（先 `Ctrl+C` 停止，再 `npm start`）。

---

### 🌐 第三步：让前端监听所有网络接口

默认情况下，前端开发服务器只监听 `localhost`，局域网内其他设备无法访问。

启动前端时，需要加上 `HOST=0.0.0.0` 环境变量：
...
其他设备访问 **Network 那一行的地址** 即可，即 `http://192.168.1.100:3000`。

---

### 🔥 第四步：开放 Windows 防火墙端口（Windows 用户必看）

如果其他设备仍然无法访问，可能是 Windows 防火墙拦截了端口 3000 和 5000。

1. 打开"Windows Defender 防火墙" → "高级设置"
2. 点击左边"入站规则" → 右边"新建规则"
3. 规则类型选"端口" → 下一步
4. 选"TCP"，在"特定本地端口"中填入 `3000,5000` → 下一步
5. 选"允许连接" → 下一步 → 全选三个选项 → 下一步
6. 填写规则名称（如"Attendance System"）→ 完成

**macOS** 通常不需要这一步，防火墙默认允许局域网连接。

---

### ✅ 验证是否成功

在其他设备（手机/其他电脑）上，确保连接到**同一个 Wi-Fi 网络**，然后：

1. 打开浏览器，访问 `http://192.168.1.100:3000`（替换为你的 IP）
2. 能看到登录页面 → ✅ 前端正常
3. 登录成功，能进入仪表盘 → ✅ 后端 CORS 配置正常

> 💡 **注意**：每次电脑重启后，IP 地址可能会改变。如果 IP 变了，记得更新 `backend/.env` 中的 `CLIENT_URL` 并重启后端，同时用新 IP 重启前端。
...
# 其他设备访问：http://[你的IP]:3000（查看第9节获取IP）
```
